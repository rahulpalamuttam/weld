...
def move(galaxy, dt):
    ...
    # Note: dx*m causes an evaluation always because their shapes don't match, so we offload to
    # numpy for matrix ops. But G*(dx*m) should not cause an evaluation. But if we do G*dx*m -->
    # then first G*dx is performed, and then multiplying the result with m immediately causes an
    # evaluation...
    Fx = (G*(dx*m))/r2
    Fy = (G*(dy*m))/r2
    Fz = (G*(dz*m))/r2
