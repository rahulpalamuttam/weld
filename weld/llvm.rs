...
impl LlvmGenerator {
    ...
    *********************************************************************************************/
    ...
    fn gen_nvptx_wrapper(&mut self, par_for: &ParallelForData, func: &SirFunction, 
                             ctx: &mut FunctionContext) -> WeldResult<()> {
        ...
        //let mut external_func_args :String = "".to_string();
        let mut external_func_args = vec!["-w".to_string(), "60".to_string(), "arg".to_string()];

        let elem_ty_str = self.llvm_type(&elem_ty)?;
        let mut size_str :String = "".to_owned();
        for (i, iter) in par_for.data.iter().enumerate() {
            // TODO: DO WE NEED THIS?
            let inner_elem_ty_str = if par_for.data.len() == 1 {
                elem_ty_str.clone()
            ...
            let arr_ptr = self.get_array_at_idx(ctx, data_llvm_info.clone(), false, &"0".to_owned())?;
            size_str = data_llvm_info.len_str;
            // FIXME:
            external_func_args[i] = format!("{}* {} ,", inner_elem_ty_str, arr_ptr);
        };
        ctx.code.add(format!("call void @weld_ptx_execute(double *{}, double *{}, int {})", external_func_args[0], external_func_args[1], size_str));
    ...
    pub fn gen_par_for_functions(&mut self, par_for: &ParallelForData, sir: &SirProgram, func: &SirFunction) -> WeldResult<()> {
        ...
        if par_for.innermost {
            ...
            let mut nvptx_wrapper_ctx = &mut FunctionContext::new(par_for.innermost);
            // should have same args as the original function.
            //
            self.gen_nvptx_wrapper(par_for, func, nvptx_wrapper_ctx);
            println!("{}", nvptx_wrapper_ctx.alloca_code.result());
            println!("{}", nvptx_wrapper_ctx.code.result());
        }
