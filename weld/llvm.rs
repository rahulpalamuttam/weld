...
impl LlvmGenerator {
    ...
    *********************************************************************************************/
        ...
        if !(par_for.data.len() == 1 && par_for.data[0].start.is_none()) {
            for (i, iter) in par_for.data.iter().enumerate() {
                if iter.kind == IterKind::NdIter {
                     ...
                     * contiguous */
                    let arr_llvm_info = self.get_array_llvm_info(func, ctx, &iter.data, "".to_string(), false)?;
                     ...
                     */
                    let el_type = self.llvm_type(&Scalar(I64))?;
                    /* both these have same type */
                    let shapes_llvm_info = self.get_array_llvm_info(func, ctx, iter.shapes.as_ref().unwrap(), el_type.clone(), false)?;
                    let strides_llvm_info = self.get_array_llvm_info(func, ctx, iter.strides.as_ref().unwrap(), el_type, false)?;
                    ...
                    ctx.code.add(format!("{} = icmp slt i64 {}, {}", cond, max_iter_str, arr_llvm_info.len));
    ...
    fn gen_loop_iteration_setup(&mut self,
        ...
        if nditer.is_some() {
            ...
            let shapes_el_type = self.llvm_type(&Scalar(I64))?;
            let shapes_llvm_info = self.get_array_llvm_info(func, ctx, first_iter.shapes.as_ref().unwrap(), shapes_el_type, true)?;
            /* dynamically generates an array of len(shapes) ints on the stack. There does not seem to
             * be any reason to use malloc here. */
            ctx.code.add(format!("%counter.idx = alloca i64, i64 {}", shapes_llvm_info.len));
        ...
        for (i, iter) in par_for.data.iter().enumerate() {
            ...
            let arr_llvm_info = self.get_array_llvm_info(func, ctx, &iter.data, inner_elem_ty_str.clone(), true)?;
            ...
            let arr_idx = if iter.kind == IterKind::NdIter {
                ...
                if iter.kind == IterKind::FringeIter {
                    ...
                    let offset = ctx.var_ids.next();
                    let final_idx = ctx.var_ids.next();
                    ctx.code.add(format!("{} = call i64 {}.size({} {})", arr_len, arr_llvm_info.prefix,
                                 arr_llvm_info.arr_type,arr_llvm_info.arr));
            ...
            let inner_elem_tmp = match iter.kind {
               IterKind::SimdIter => {
                    self.get_array_at_idx(ctx, arr_llvm_info, true, &arr_idx)?
               }
               /* General case for ScalarIter, NdIter and FringeIter */
               _ => {
                    self.get_array_at_idx(ctx, arr_llvm_info, false, &arr_idx)?
               }
            };
            if par_for.data.len() == 1 {
                prev_ref.clear();
    ...
    fn gen_loop_iteration_end(&mut self,
        ...
        if nditer.is_some() {
            ...
            let shapes_el_type = self.llvm_type(&Scalar(I64))?;
            let shapes_llvm_info = self.get_array_llvm_info(func, ctx, iter.shapes.as_ref().unwrap(), shapes_el_type, true)?;
            ctx.code.add(format!("{i} = alloca i64
                                 ...
                                 , i="%counter.i", counter_len=shapes_llvm_info.len,
    ...
    *********************************************************************************************/
    ...
    fn gen_nvptx_func(&mut self, par_for: &ParallelForData, func: &SirFunction,
        ...
        ctx.code.add(format!("{} = getelementptr {}, {}* null, i32 1", &elem_size_ptr, &output_ty, &output_ty));
        ctx.code.add(format!("{} = ptrtoint {}* {} to i64", &elem_size, &output_ty, &elem_size_ptr));
        ...
        ctx.code.add(format!("{} = bitcast i8* {} to {}*", &elem_storage_typed, &elem_storage, &output_ty));
